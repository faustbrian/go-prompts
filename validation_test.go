...
func TestPromptDefensivelyCopiesValidationPipeline(t *testing.T) {
	...
	validators := []prompts.Validator[string]{
...
func TestValidationPipelineRejectsNilCallbacks(t *testing.T) {
	...
	for _, config := range tests {
		...
		if !errors.Is(err, prompts.ErrInvalidDefinition) {
			t.Fatalf(
				"NewText(%q) error = %v, want ErrInvalidDefinition",
				config.ID,
				err,
			)
		}
...
func TestValidationIssueZeroAndDefensiveBehavior(t *testing.T) {
	...
	if nilIssue.Error() != "validation failed" ||
		nilIssue.Code() != "" ||
		nilIssue.Message() != "" ||
		nilIssue.Fields() != nil {
	...
	if issue.Code() != "bad�" ||
		...
		issue.Fields()[0] != "field�" {
		t.Fatalf(
			"issue content was not neutralized: %q %q %#v",
			issue.Code(),
			issue.Message(),
			issue.Fields(),
		)
	...
	for _, test := range tests {
		t.Run(
			...
			func(t *testing.T) {
				...
				if issue.Code() != "validation" ||
					issue.Message() != "unsafe� failure" {
					t.Fatalf(
						"normalized issue = %q %q",
						issue.Code(),
						issue.Message(),
					)
				}
...
func (ctx *stagedContext) Value(any) any {
	return nil
}
...
func (emptyValidationError) Error() string {
	return ""
}

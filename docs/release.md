...
A release must start from a clean clone with `GOWORK=off`, pass `make check`,
retained fuzz budgets, mutation review,
...
entry points. The API export baseline is exact for v1; an intentional public
change must follow semantic versioning and update it with review.

Signed `v*` tags trigger the release workflow. It verifies the tag,

...
func FuzzFormNavigationAndVisibilityMatchesModel(fuzz *testing.F) {
	...
	fuzz.Fuzz(
		func(t *testing.T, rawName, rawDetails []byte, revisit bool) {
			if len(rawName) > 128 || len(rawDetails) > 128 {
				t.Skip()
			}
			name := hex.EncodeToString(rawName)
			details := hex.EncodeToString(rawDetails)
			namePrompt, err := prompts.NewText(
				prompts.TextConfig{ID: "name", Label: "Name"},
			)
			if err != nil {
				t.Fatal(err)
			}
			...
			detailsPrompt, err := prompts.NewText(
				prompts.TextConfig{ID: "details", Label: "Details"},
			)
			if err != nil {
				t.Fatal(err)
			}
			...
			if !nameOK ||
				gotName != name ||
				!advancedOK ||
				gotAdvanced != wantAdvanced {
				t.Fatalf(
					"result = %v, name %q, advanced %t",
					result.IDs(),
					gotName,
					gotAdvanced,
				)
		...
		func(t *testing.T, raw []byte) {
			...
			for _, value := range raw {
				...
				case 2:
					...
					wantApplied := generation != 0 &&
						generation == current &&
						now >= due
					if applied != wantApplied {
						t.Fatalf(
							"Resolve(%d) applied = %t; want %t",
							generation,
							applied,
							wantApplied,
						)
					...
					if wantApplied {
						...
						if len(options) != 1 ||
							options[0].ID() != appliedQuery {
							t.Fatalf(
								"Resolve() options = %#v; want %q",
								options,
								appliedQuery,
							)
						}
				...
				if generation != current ||
					(appliedQuery == "" && len(snapshot) != 0) ||
					(appliedQuery != "" &&
						(len(snapshot) != 1 ||
							snapshot[0].ID() != appliedQuery)) {
					t.Fatalf(
						"Snapshot() = %#v, %d; model = %q, %d",
						snapshot,
						generation,
						appliedQuery,
						current,
	...
	fuzz.Fuzz(
		func(t *testing.T, raw []byte) {
			...
			progress, err := prompts.NewProgress(
				prompts.ProgressConfig{ID: "work", Label: "Work", Total: 32},
			)
			if err != nil {
				t.Fatal(err)
			}
			...
			for index, value := range raw {
				...
				if snapshot.Current != current ||
					...
					snapshot.Total != 32 {
					t.Fatalf(
						"operation %d snapshot = %#v; model current=%d state=%d",
						index,
						snapshot,
						current,
						state,

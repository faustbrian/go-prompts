...
func TestOwnOptionsExactBoundsAndRequiredFields(t *testing.T) {
	...
	for name, option := range map[string]Option[int]{"identity": {label: "One"}, "label": {id: "one"}} {
...
func TestMultiSelectAcceptsExactDeclaredBounds(t *testing.T) {
	...
	prompt, err := NewMultiSelect(
		MultiSelectConfig[int]{
			ID:         "numbers",
			Label:      "Numbers",
			Options:    options,
			Min:        2,
	...
	if err != nil ||
		prompt.definition.selection.minimum != 2 ||

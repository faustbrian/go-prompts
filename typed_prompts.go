...
func NewMultiline(config MultilineConfig) (Prompt[string], error) {
	return newStringPrompt(
		KindMultiline,
		"define multiline prompt",
		stringDefinitionConfig{
			config.ID,
			config.Label,
			config.Description,
			config.Placeholder,
...
func newStringPrompt(
	kind PromptKind,
	operation string,
	config stringDefinitionConfig,
	parser func(string) (string, error),
) (Prompt[string], error) {
	if config.id == "" || config.label == "" {
		return Prompt[string]{}, invalidBehaviorDefinition(
			operation,
			config.id,
			fmt.Errorf("%w: identity and label are required", ErrInvalidDefinition),
		)
	}
	for _, callback := range config.pre {
		if callback == nil {
			return Prompt[string]{}, invalidBehaviorDefinition(
				operation,
				config.id,
				fmt.Errorf("%w: nil pre-validator", ErrInvalidDefinition),
			)
		}
	}
	for _, callback := range config.transform {
		if callback == nil {
			return Prompt[string]{}, invalidBehaviorDefinition(
				operation,
				config.id,
				fmt.Errorf("%w: nil transformer", ErrInvalidDefinition),
			)
		}
	}
	for _, callback := range config.post {
		if callback == nil {
			return Prompt[string]{}, invalidBehaviorDefinition(
				operation,
				config.id,
				fmt.Errorf("%w: nil post-validator", ErrInvalidDefinition),
			)
		}
	}
...
func NewPath(config PathConfig) (Prompt[Path], error) {
	if config.Kind > PathDirectory {
		return Prompt[Path]{}, invalidBehaviorDefinition(
			"define path prompt",
			config.ID,
			fmt.Errorf("%w: invalid path kind", ErrInvalidDefinition),
		)
	}
...
func NewConfirm(config ConfirmConfig) (Prompt[bool], error) {
	...
	for _, value := range accept {
		...
		if _, duplicate := values[normalized]; normalized == "" || duplicate {
			return Prompt[bool]{}, invalidBehaviorDefinition(
				"define confirm prompt",
				config.ID,
				fmt.Errorf(
					"%w: invalid or duplicate confirmation value",
	...
	for _, value := range reject {
		...
		if _, duplicate := values[normalized]; normalized == "" || duplicate {
			return Prompt[bool]{}, invalidBehaviorDefinition(
				...
				config.ID,
				fmt.Errorf(
					"%w: overlapping confirmation values",
					ErrInvalidDefinition,
				),
			)
		}
...
func newTypedPrompt[T any](
	kind PromptKind,
	operation, id, label, description, placeholder, hint, help string,
	defaultValue, fallbackValue Optional[T],
	headless HeadlessBehavior,
	accessibility Accessibility,
	pre []Validator[T],
...
func Parse[T any](
	...
	var zero T
	if ctx == nil {
		return zero, invalidBehaviorDefinition(
			"parse prompt",
			prompt.ID(),
			ErrInvalidDefinition,
		)

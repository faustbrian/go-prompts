		...
		func(t *testing.T, raw []byte) {
			...
			insert := func(value string) {
				...
				if wantError {
					if err == nil {
						t.Fatalf(
							"insert %q exceeded the model byte bound",
							value,
						)
					}
					return
			...
			for index, value := range raw {
				...
				case 11:
					...
					for cursor < len(cells) &&
						strings.TrimSpace(cells[cursor]) != "" {
						cursor++
					}
					for cursor < len(cells) &&
						strings.TrimSpace(cells[cursor]) == "" {
				...
				if editor.cursor != cursor ||
					editor.text() != strings.Join(cells, "") {
					t.Fatalf(
						"operation %d editor = %q at %d; model = %q at %d",
						index,
						editor.text(),
						editor.cursor,
						strings.Join(cells, ""),
...
func assertSelectionModel(
	...
	t.Helper()
	if len(state.visible) != len(visible) ||
		state.focus != focus ||
		len(state.selected) != len(selected) {
		t.Fatalf(
			"operation %d state = %#v; model visible=%v focus=%d selected=%v",
			operation,
			state,
			visible,
			focus,
	...
	for _, option := range options {
		if state.selected[option.id] != selected[option.id] {
			t.Fatalf(
				"operation %d selection %q = %t; model = %t",
				operation,
				option.id,

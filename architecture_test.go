...
func TestCoreDependencyAndRuntimeBoundaries(t *testing.T) {
	...
	for _, entry := range entries {
		...
		for _, imported := range file.Imports {
			...
			if slices.Contains([]string{"os", "os/exec"}, path) ||
				strings.Contains(path, "charm") ||
				strings.Contains(path, "survey") ||
				strings.Contains(path, "promptui") {
		...
		ast.Inspect(
			...
			func(node ast.Node) bool {
				...
				case *ast.GoStmt:
					t.Errorf(
						"%s launches an unowned goroutine at %d",
						name,
						value.Pos(),
					)
				case *ast.FuncDecl:
					if value.Name.Name == "init" {
						t.Errorf(
							"%s defines package initialization side effects",
							name,
						)
					}
				case *ast.CallExpr:

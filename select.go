...
func NewOption[T any](config OptionConfig[T]) (Option[T], error) {
	if config.ID == "" || config.Label == "" {
		return Option[T]{}, invalidBehaviorDefinition(
			"define option",
			config.ID,
			fmt.Errorf(
				"%w: option identity and label are required",
...
func (option Option[T]) ID() string {
	return option.id
}

func (option Option[T]) Label() string {
	return option.label
}

func (option Option[T]) Description() string {
	return option.description
}

func (option Option[T]) Group() string {
	return option.group
}
...
func (option Option[T]) Disabled() bool {
	...
	if config.InitialID != "" {
		...
		if !exists || option.disabled {
			return Prompt[T]{}, invalidBehaviorDefinition(
				"define select prompt",
				config.ID,
				fmt.Errorf("%w: invalid initial option", ErrInvalidDefinition),
			)
		}
	}
...
func ownOptions[T any](source []Option[T], maximum int) ([]Option[T], map[string]Option[T], error) {
	...
	if maximum < 1 || len(source) == 0 || len(source) > maximum {
		return nil, nil, fmt.Errorf(
			"%w: option count is outside configured bounds",
			ErrInvalidDefinition,
		)
	}
...
func resolveOption[T any](
	identity Optional[string],
	byID map[string]Option[T],
) (Optional[T], error) {
	...
	if !exists || option.disabled {
		return Optional[T]{}, fmt.Errorf(
			"%w: default or fallback option is unavailable",
			ErrInvalidDefinition,
		)
	}
...
func NewMultiSelect[T any](config MultiSelectConfig[T]) (Prompt[[]T], error) {
	...
	if config.Min < 0 || maximum < config.Min || maximum > len(options) {
		return Prompt[[]T]{}, invalidBehaviorDefinition(
			"define multi-select prompt",
			config.ID,
			fmt.Errorf("%w: invalid selection bounds", ErrInvalidDefinition),
		)
	}
	...
	post = append(
		post,
		func(_ context.Context, values []T, _ ValidationContext) error {
	...
	prompt.definition.clone = func(values []T) []T {
...
func resolveOptionIDs[T any](
	identities Optional[[]string],

				...
				func(
					...
					if len(values) != 2 {
						return prompts.NewValidationIssue(
							"choose_two",
							"Select exactly two choices with Space before pressing Enter",
							"review-multi",
						)
					}
			...
			PostValidate: []prompts.Validator[string]{
				func(
					...
					if len(strings.TrimSpace(value)) < 2 {
						return prompts.NewValidationIssue(
							"too_short",
							"Type at least two letters before pressing Enter",
							"review-name",
						)
					}
					return nil
...
func message(ctx context.Context, kind prompts.MessageKind, title, body string) error {
	return prompts.WriteMessage(
		ctx,
		prompts.Message{Kind: kind, Title: title, Body: body},
		outputExecution(),
	)
}

...
func FuzzPlainRenderingIsDeterministic(fuzz *testing.F) {
	...
	fuzz.Fuzz(
		func(t *testing.T, label, value string, rawWidth uint8) {
			width := int(rawWidth % 80)
			frame := prompts.NewFrame(
				prompts.Line(
					prompts.Text(prompts.RoleLabel, label),
					prompts.Text(prompts.RoleValue, value),
				),
			)
			first, err := (prompts.PlainRenderer{}).Render(
				frame,
				prompts.RenderOptions{Width: width},
			)
			if err != nil {
				t.Fatalf("Render() error = %v", err)
			}
			second, err := (prompts.PlainRenderer{}).Render(
				frame,
				prompts.RenderOptions{Width: width},
			)
			if err != nil || first != second {
				t.Fatalf(
					"Render() was nondeterministic: %q, %q, %v",
					first,
					second,
					err,
				)
...
func FuzzHyperlinkNeverInjectsTerminalControls(fuzz *testing.F) {
	...
	fuzz.Fuzz(
		func(t *testing.T, label, target string) {
...
func FuzzInteractiveSecretBytesNeverRendersInput(fuzz *testing.F) {
	...
	fuzz.Fuzz(
		func(t *testing.T, raw []byte) {
			...
			terminal.Push(
				prompts.PasteBytesEvent(canary),
	...
	fuzz.Fuzz(
		func(t *testing.T, raw []byte) {
			...
			prompt, err := prompts.NewText(
				prompts.TextConfig{ID: "text", Label: "Text"},
			)
			if err != nil {
				t.Fatal(err)
			}
			...
			for _, value := range raw {
				...
				case 11:
					events = append(
						events,
						prompts.KeyEvent(prompts.KeyIgnored),
					)
				case 12:
					events = append(
						events,
	...
	fuzz.Fuzz(
		func(t *testing.T, query string) {
			...
			if err != nil || !equalStrings(optionIDs(first), optionIDs(second)) {
				t.Fatalf(
					"Search() was nondeterministic: %q, %q, %v",
					optionIDs(first),
					optionIDs(second),
					err,
				)
...
func FuzzSecretEntryFailureNeverDiscloses(fuzz *testing.F) {
	...
	fuzz.Fuzz(
		func(t *testing.T, raw []byte, outcome uint8) {
			if len(raw) > 512 {
				t.Skip()
			}
			...
			for _, candidate := range allowed {
				if candidate == nil && runErr == nil ||
					candidate != nil && errors.Is(runErr, candidate) {
					matched = true
					break
				}
			}
			...
			if !terminal.Released() || !terminal.EchoEnabled() {
				t.Fatalf(
					"terminal was not restored: released=%t echo=%t",
					terminal.Released(),

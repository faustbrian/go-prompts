...
func TestProgressCoalescesConcurrentUpdatesAndRendersStableLines(t *testing.T) {
	...
	progress, err := prompts.NewProgress(
		prompts.ProgressConfig{ID: "upload", Label: "Upload", Total: 100},
	)
	if err != nil {
	...
	for value := int64(1); value <= 100; value++ {
		group.Go(
			func() {
				_ = progress.Update(value, "uploading")
			},
		)
	}
	group.Wait()
	...
	if snapshot.Current < 1 ||
		...
		snapshot.Total != 100 ||
		snapshot.State != prompts.ProgressRunning {
	...
	if !strings.Contains(output, "success: Upload: 100/100 (100%) - done") ||
...
func TestProgressRejectsRegressionOverflowAndTerminalMutation(t *testing.T) {
	...
	progress, err := prompts.NewProgress(
		prompts.ProgressConfig{ID: "work", Label: "Work", Total: 10},
	)
	if err != nil {
		t.Fatalf("NewProgress() error = %v", err)
	}
	...
	bounded, err := prompts.NewProgress(
		prompts.ProgressConfig{ID: "bound", Label: "Bound", Total: 10},
	)
	if err != nil {
		t.Fatalf("NewProgress() error = %v", err)
	}
...
func TestCallerDrivenMutationsHonorContext(t *testing.T) {
	...
	progress, err := prompts.NewProgress(
		prompts.ProgressConfig{ID: "copy", Label: "Copy", Total: 3},
	)
	if err != nil {
		t.Fatalf("NewProgress() error = %v", err)
	}
...
func TestProgressAllowsExplicitRegressionAndUnknownTotal(t *testing.T) {
	...
	progress, err := prompts.NewProgress(
		prompts.ProgressConfig{ID: "scan", Label: "Scan", AllowRegression: true},
	)
	if err != nil {
		t.Fatalf("NewProgress() error = %v", err)
	}
...
func TestProgressCalculatesExplicitClockRateAndETA(t *testing.T) {
	...
	progress, err := prompts.NewProgress(
		prompts.ProgressConfig{ID: "download", Label: "Download", Total: 10, Clock: clock},
	)
	if err != nil {
		t.Fatalf("NewProgress() error = %v", err)
	}
...
func TestProgressDefinitionsAndRenderingFailuresAreTyped(t *testing.T) {
	...
	progress, err := prompts.NewProgress(
		prompts.ProgressConfig{ID: "work", Label: "Work", Total: 1},
	)
	if err != nil {
		t.Fatalf("NewProgress() error = %v", err)
	}
...
func TestSpinnerIsCallerDrivenAndReducedMotionSafe(t *testing.T) {
	...
	states := []struct {
		...
		},
		{
			"cancel",
			func(value *prompts.Spinner) {
				value.Cancel("stopped")
			},
			"warning:",
		},
	}
	for _, state := range states {
		value, createErr := prompts.NewSpinner(
			prompts.SpinnerConfig{ID: state.name, Label: state.name},
		)
		if createErr != nil {
